...
void print() {
	...
	cout << "\n>> @ current ans is " << ans << endl;
	cout << "\n=============\n";
}
coor dir[4][9] = {
	{{-2,-1,0.02},{-1,-2,0.1},{-1,-1,0.07},{-1,0,0.01},{0,-3,0.05},{1,-2,0.1},{1,-1,0.07},{1,0,0.01},{-1,2,0.02} },//left
...
void move(int k) {
	...
	for (int i = 0; i < 9; i++) {
		...
		tmpR = curR + dir[k][i].x;
		tmpC = curC + dir[k][i].y;
		if (isExist(tmpR, tmpC)) {//해당 좌표가 존재하면 비율만큼의 모래를 해당 좌표에 더하고 y위치에서 빼준다
			a[tmpR][tmpC] += tmpV;
		}
	
		
		else {//존재하지 않으면 밖으로 나간 것, 답에 더해준다
			ans += tmpV;
		}
		sum += tmpV;
	}
