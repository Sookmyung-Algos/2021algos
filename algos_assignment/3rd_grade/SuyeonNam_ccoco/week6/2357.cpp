...
long long max_init(vector<long long> &a, vector<long long> &tree, long long node, long long start, long long end) {
    if (start == end) {
...
long long max_f(vector<long long> &tree, long long node, long long start, long long end, long long left, long long right) {
    if (left > end || right < start) return 0;
    if (left <= start && end <= right) return tree[node];
    return max(max_f(tree, node * 2, start, (start + end) / 2, left, right), max_f(tree, node * 2 + 1, (start + end) / 2 + 1, end, left, right));
}
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = 0; i < m; i++) {
        cin >> s >> t;
        cout << min_f(min_tree, 1, 0, n-1, s-1, t-1) << ' ' << max_f(max_tree, 1, 0, n-1, s-1, t-1) << '\n';
    }
    return 0;
}
