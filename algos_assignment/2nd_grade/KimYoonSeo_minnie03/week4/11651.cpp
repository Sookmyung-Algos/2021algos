...
void merge(int left, int mid, int right) {
	...
	while (left <= left_end && mid <= right) {
		if (arr[left].y < arr[mid].y) //arr배열의 맨왼쪽 원소의 y좌표가 mid의 y좌표보다 작으면
			temp[tmp_pos++] = arr[left++]; //temp배열에 arr맨왼쪽의 원소 삽입
		else if (arr[left].y > arr[mid].y) //arr배열의 맨왼쪽 원소의 y좌표가 mid의 y좌표보다 크면
			temp[tmp_pos++] = arr[mid++]; //temp배열에 arr배열의 mid 원소 삽입
		else { //arr배열의 맨왼쪽 원소의 y좌표와 mid의 y좌표가 같으면
			if (arr[left].x < arr[mid].x) //x좌표를 비교하여 left의 x좌표가 더 작으면
				temp[tmp_pos++] = arr[left++]; //temp배열에 arr배열의 left 원소 삽입
			else //mid의 x좌표가 더 작으면
				temp[tmp_pos++] = arr[mid++]; //temp배열에 arr배열의 mid 원소 삽입
		}
	}
